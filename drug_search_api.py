...
def get_drug_info(item_name):
    ...
    params = {
        ...
        'numOfRows': 1,
        'type': 'json',
        'item_name': item_name  # 함수의 인자로 받은 약품명 사용
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return product_name, serial_num
        else:
...
def get_drug_info(item_names):
    ...
    for item_name in item_names:
        item_name = item_name.strip() # 공백제거
        ...
        params = {
            ...
            'numOfRows': 1,
            'type': 'json',
            'item_name': encoded_item_name  # 리스트의 각 약품명 사용
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                product_name = data['body']['items'][0]['ITEM_NAME']
                serial_num = data['body']['items'][0]['ITEM_SEQ']
                results.append((product_name, serial_num))
            else:
                print(f"에러: {response.status_code}")
                print("응답 내용:", response.text)
